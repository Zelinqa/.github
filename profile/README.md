<div align="center">
  <a href="https://zelinqa.ai">
    <img src="./assets/zelinqa-logo.svg" alt="Zelinqa" width="360" />
  </a>

  <h2>The Questioning Intelligence Layer for AI</h2>

  <p><strong>AI knows how to answer. We help it know what to ask next.</strong></p>

  <p>
    Zelinqa gives AI agents the intelligence to select the next best question,
    understand conversational progress, and guide every exchange toward a goal.
  </p>
</div>

<br />

<div align="center">
  <img
    src="./assets/nbq-product-map.png"
    alt="Your AI agent connects to the NBQ Engine through the Python and TypeScript SDKs or the MCP server to create better conversations."
    width="100%"
  />
</div>

<p align="center">
  Goal-driven question selection, contextual progression, and conversation
  outcomes — exposed through one API.
</p>

<p align="center"><strong>Start building</strong></p>

<!--
  GitHub force les tableaux en `display: block; width: max-content` : les
  attributs width (%, 100%) sont ignorés, seule la largeur du CONTENU compte.
  L'espaceur transparent de 290 px en fin de cellule est donc ce qui fixe des
  colonnes égales et un tableau pleine largeur. `valign="top"` + une structure
  identique dans les trois cellules alignent badges, commandes et liens.
-->
<table>
  <tr>
    <td align="center" valign="top" width="33%">
      <img
        src="https://img.shields.io/badge/Python_SDK-0B1F33?style=for-the-badge&logo=python&logoColor=8B5CF6"
        alt="Python SDK"
      /><br /><br />
      <img
        src="https://img.shields.io/badge/pip_install_nbq-F4F0FF?style=flat-square&logo=pypi&logoColor=6D28D9&labelColor=F4F0FF&color=F4F0FF"
        alt="pip install nbq"
      /><br /><br />
      <a href="https://pypi.org/project/nbq/">PyPI</a>
      &nbsp;·&nbsp;
      <a href="https://github.com/Zelinqa/nbq-sdk">Source</a>
      <br />
      <img src="./assets/spacer.svg" width="290" height="1" alt="" />
    </td>
    <td align="center" valign="top" width="33%">
      <img
        src="https://img.shields.io/badge/TypeScript_SDK-0B1F33?style=for-the-badge&logo=typescript&logoColor=8B5CF6"
        alt="TypeScript SDK"
      /><br /><br />
      <img
        src="https://img.shields.io/badge/pnpm_add_%40zelinqa%2Fnbq-F4F0FF?style=flat-square&logo=pnpm&logoColor=6D28D9&labelColor=F4F0FF&color=F4F0FF"
        alt="pnpm add @zelinqa/nbq"
      /><br /><br />
      <a href="https://www.npmjs.com/package/@zelinqa/nbq">npm</a>
      &nbsp;·&nbsp;
      <a href="https://github.com/Zelinqa/nbq-sdk">Source</a>
      <br />
      <img src="./assets/spacer.svg" width="290" height="1" alt="" />
    </td>
    <td align="center" valign="top" width="33%">
      <img
        src="https://img.shields.io/badge/Official_MCP_Server-0B1F33?style=for-the-badge&logoColor=8B5CF6"
        alt="Official MCP Server"
      /><br /><br />
      <img
        src="https://img.shields.io/badge/AI_agent_tools-F4F0FF?style=flat-square&logoColor=6D28D9&labelColor=F4F0FF&color=F4F0FF"
        alt="AI agent tools"
      /><br /><br />
      <a href="https://github.com/Zelinqa/nbq-mcp"><code>nbq-mcp</code></a>
      <br />
      <img src="./assets/spacer.svg" width="290" height="1" alt="" />
    </td>
  </tr>
</table>

<p align="center">
  <a href="https://docs.zelinqa.ai"><strong>Documentation</strong></a>
  &nbsp;&nbsp;·&nbsp;&nbsp;
  <a href="https://zelinqa.ai"><strong>Website</strong></a>
  &nbsp;&nbsp;·&nbsp;&nbsp;
  <a href="mailto:contact@zelinqa.ai"><strong>Contact</strong></a>
</p>

---

<p align="center">
  <sub>
    The NBQ SDK and MCP server are open source. The hosted NBQ Engine, scoring
    algorithm, prompts, and infrastructure remain private.
  </sub>
</p>
